...
public class ExcelStepDefinitions {
    ...
    @And("ingilizce Ve Turkce Adlari Ayni Olan")
    public void ingilizceVeTurkceAdlariAyniOlan() {
        int count = 1;
        int actualFizKulSatir = workbook.getSheet("Sayfa1").getPhysicalNumberOfRows(); //190 ulke
        for (int i = 0; i <= actualFizKulSatir; i++) {

            if (workbook.getSheet("Sayfa1").getRow(i).getCell(1).toString()
                    .equals
                            (workbook.getSheet("Sayfa1").getRow(i).getCell(3).toString())) {
                System.out.println(count + ". " + workbook.getSheet("Sayfa1").getRow(i).getCell(2).toString() + " / "
